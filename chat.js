...
const GEMINI_MODEL = "gemini-flash-latest"; // alias: always the current stable Flash
...
const RENDER_PAGE_TOOL = {
  ...
  input_schema: {
    ...
    required: ["layout", "title", "subtitle", "intro"],
  },
};
...
function normalizeSpec(raw) {
  ...
  if (layout === "off-topic") {
    return { layout, title: title || "off-topic.", subtitle: subtitle || "", intro: "", items: [], big: big || OFF_TOPIC_FALLBACK.big };
  }

  if (layout === "about" || layout === "contact") {
    return { layout, title: title || "", subtitle: subtitle || "", intro: intro || "", items: [] };
  }
  ...
  if (forcedLayout) {
    return { layout: forcedLayout, title: title || "", subtitle: subtitle || "", intro: intro || "", items: [items[0]] };
  }

  if (layout === "case-study") return { layout, title: title || "weave.", subtitle: subtitle || "autodesk / weave", intro: "", items: ["weave"] };
  if (layout === "manyfest-case") return { layout, title: title || "manyfest.", subtitle: subtitle || "manychat design system", intro: "", items: ["manychat-ds"] };
  if (layout === "releve-case") return { layout, title: title || "(r)elevē.", subtitle: subtitle || "elisava capstone", intro: "", items: ["releve"] };

  if (layout === "single-project") {
    const id = items.find((x) => DETAIL_IDS.includes(x) && !CASE_LAYOUT_FOR_ID[x]);
    if (!id) return GALLERY_FALLBACK;
    return { layout, title: title || "", subtitle: subtitle || "", intro: intro || "", items: [id] };
  }

  if (layout === "gallery" || layout === "quality-showcase" || layout === "themed-list") {
    const valid = items.filter((x) => GALLERY_IDS.includes(x));
    if (!valid.length) return GALLERY_FALLBACK;
    return { layout, title: title || "selected work.", subtitle: subtitle || "", intro: intro || "", items: valid };
  }
